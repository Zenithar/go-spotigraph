...
var fileDescriptor_8d70e9cb66fdadf5 = []byte{
	...
	0x33, 0x5c, 0xc4, 0xc4, 0x1c, 0xec, 0x1e, 0xb1, 0x8a, 0x49, 0x38, 0x18, 0xa1, 0x05, 0xac, 0x50,
	0x2f, 0xcc, 0xf0, 0x14, 0xb2, 0x68, 0x0c, 0x58, 0x34, 0x26, 0xcc, 0x30, 0x89, 0x0d, 0xec, 0x2b,
	0x63, 0x40, 0x00, 0x00, 0x00, 0xff, 0xff, 0x3a, 0xc9, 0xd8, 0x6f, 0xf8, 0x00, 0x00, 0x00,
}
...
func (m *Guild) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	...
	if len(m.Urn) > 0 {
		i -= len(m.Urn)
		copy(dAtA[i:], m.Urn)
		i = encodeVarintGuild(dAtA, i, uint64(len(m.Urn)))
		i--
		dAtA[i] = 0x1a
	}
	if len(m.Label) > 0 {
		i -= len(m.Label)
		copy(dAtA[i:], m.Label)
		i = encodeVarintGuild(dAtA, i, uint64(len(m.Label)))
		i--
		dAtA[i] = 0x12
	}
	if len(m.Id) > 0 {
