...
var UpdateHandler = func(squads repositories.Squad) reactor.HandlerFunc {
	return func(ctx context.Context, r interface{}) (interface{}, error) {
		res := &squadv1.UpdateResponse{}

		// Check request type
		req, ok := r.(*squadv1.UpdateRequest)
		if !ok {
		...
		if req.Label != nil {
			if err := constraints.Validate(ctx,
				...
				// Is already used ?
				constraints.SquadLabelMustBeUnique(squads, req.Label.Value),
			); err != nil {
				res.Error = &systemv1.Error{
		...
		if updated {
			if err := squads.Update(ctx, &entity); err != nil {
				res.Error = &systemv1.Error{
