...
func TestSquad_Get(t *testing.T) {
	// Testcases
	testCases := []struct {
		name    string
		req     interface{}
		...
		},
		{
			...
			prepare: func(ctx context.Context, squads *mock.MockSquad) {
				squads.EXPECT().Get(gomock.Any(), "0NeNLNeGwxRtS4YPzM2QV4suGMs6Q55e").Return(nil, db.ErrNoModification).Times(1)
			},
			wantErr: true,
		},
		{
	...
	for _, tt := range testCases {
		...
		t.Run(testCase.name, func(t *testing.T) {
			...
			// Prepare service
			underTest := commands.GetHandler(squads)
