...
var LeaveHandler = func(squads repositories.SquadRetriever, persons repositories.PersonRetriever, memberships repositories.Membership) reactor.HandlerFunc {
	return func(ctx context.Context, r interface{}) (interface{}, error) {
		res := &squadv1.LeaveResponse{}

		// Check request type
		req, ok := r.(*squadv1.LeaveRequest)
		if !ok {
		...
		if err := memberships.Leave(ctx, &person, &squad); err != nil {
			res.Error = &systemv1.Error{
				Code:    http.StatusInternalServerError,
				Message: "Unable to leave Squad",
			}
			return res, errors.Newf(errors.Internal, err, "unable to leave squad")
		}
