...
var SearchHandler = func(squads repositories.SquadRetriever) reactor.HandlerFunc {
	return func(ctx context.Context, r interface{}) (interface{}, error) {
		res := &squadv1.SearchResponse{}

		// Check request type
		req, ok := r.(*squadv1.SearchRequest)
		if !ok {
		...
		// Do the search
		entities, total, err := squads.Search(ctx, filter, pagination, sortParams)
