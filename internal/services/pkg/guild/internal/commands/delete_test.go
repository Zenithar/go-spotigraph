...
func TestGuild_Delete(t *testing.T) {
	...
	testCases := []struct {
		...
		// ---------------------------------------------------------------------
		{
			...
			prepare: func(ctx context.Context, guilds *mock.MockGuild) {
				...
				guilds.EXPECT().Delete(gomock.Any(), gomock.Any()).Return(nil).Times(1)
			},
			wantErr: false,
		}, {
			...
			prepare: func(ctx context.Context, guilds *mock.MockGuild) {
				u1 := models.NewGuild("Foo")
				guilds.EXPECT().Get(gomock.Any(), "0NeNLNeGwxRtS4YPzM2QV4suGMs6Q55e").Return(u1, nil).Times(1)
				guilds.EXPECT().Delete(gomock.Any(), gomock.Any()).Return(db.ErrNoResult).Times(1)
			},
			wantErr: true,
		},
	}
	...
	for _, tt := range testCases {
		...
		t.Run(testCase.name, func(t *testing.T) {
			...
			// Prepare service
			underTest := commands.DeleteHandler(guilds)
