...
func TestPerson_Delete(t *testing.T) {
	...
	testCases := []struct {
		...
		// ---------------------------------------------------------------------
		{
			...
			prepare: func(ctx context.Context, persons *mock.MockPerson) {
				...
				persons.EXPECT().Delete(gomock.Any(), gomock.Any()).Return(nil).Times(1)
			},
			wantErr: false,
		}, {
			...
			prepare: func(ctx context.Context, persons *mock.MockPerson) {
				u1 := models.NewPerson("Foo")
				persons.EXPECT().Get(gomock.Any(), "0NeNLNeGwxRtS4YPzM2QV4suGMs6Q55e").Return(u1, nil).Times(1)
				persons.EXPECT().Delete(gomock.Any(), gomock.Any()).Return(db.ErrNoResult).Times(1)
			},
			wantErr: true,
		},
	}
	...
	for _, tt := range testCases {
		...
		t.Run(testCase.name, func(t *testing.T) {
			...
			// Prepare service
			underTest := commands.DeleteHandler(persons)
