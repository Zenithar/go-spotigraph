...
var SearchHandler = func(persons repositories.PersonRetriever) reactor.HandlerFunc {
	return func(ctx context.Context, r interface{}) (interface{}, error) {
		res := &personv1.SearchResponse{}

		// Check request type
		req, ok := r.(*personv1.SearchRequest)
		if !ok {
		...
		// Build search filter
		filter := &repositories.PersonSearchFilter{}

		// Do the search
		entities, total, err := persons.Search(ctx, filter, pagination, sortParams)
