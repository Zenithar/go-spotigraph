...
func Build(name, packageName string, opts ...BuildOption) func() error {
	...
	return func() error {
		...
		varsSetByLinker := map[string]string{
			"go.zenithar.org/spotigraph/build/version.Version":   version,
			"go.zenithar.org/spotigraph/build/version.Revision":  git.Revision,
			"go.zenithar.org/spotigraph/build/version.Branch":    git.Branch,
			"go.zenithar.org/spotigraph/build/version.BuildUser": os.Getenv("USER"),
			"go.zenithar.org/spotigraph/build/version.BuildDate": time.Now().Format(time.RFC3339),
			"go.zenithar.org/spotigraph/build/version.GoVersion": runtime.Version(),
		}
		...
		return sh.RunWith(env, "go", "build", "-mod=vendor", fmt.Sprintf("-ldflags='%s'", ldflagsValue), "-o", filename, packageName)
	}
}
