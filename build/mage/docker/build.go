...
func Build(cmd *Command) func() error {
	return func() error {
		...
		err = sh.RunWith(
			...
			"/bin/sh", "-c",
			fmt.Sprintf("echo '%s' | base64 -D | docker build -t spotigraph/%s -f- --build-arg BUILD_DATE=%s --build-arg VERSION=%s --build-arg VCS_REF=%s .", base64.StdEncoding.EncodeToString(buf.Bytes()), cmd.Bin, time.Now().Format(time.RFC3339), git.Tag, git.Revision),
		)
